...
SEED_IMAGE_DIR = Path(__file__).resolve().parents[2] / "seed_images" / "products"
...
PRODUCTS = [
    ...
    },
    {
        ...
        "description": (
            "Screen replacement service for common 13-inch to 16-inch laptops."
        ),
        "tech_specs": {
    ...
    },
    {
        ...
        "description": (
            "Hardware diagnostics, cleanup, and performance tune-up for desktop PCs."
        ),
        "tech_specs": {
    ...
    },
    {
        ...
        "description": (
            "Printer setup, driver installation, paper-feed checks, and basic repair."
        ),
        "tech_specs": {
...
class Command(BaseCommand):
    ...
    def seed_order(self, tenant, user, active_cart, variants):
        ...
        for cart_item in CartItem.objects.filter(cart=order_cart):
            ...
            variant_label = ", ".join(
                value
                for value in [variant.color, variant.storage, variant.ram]
                if value
            )
