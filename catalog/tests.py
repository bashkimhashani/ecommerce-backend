...
class ProductDetailSerializerTests(APITestCase):
    ...
    def test_product_detail_serializer_returns_nested_product_fields(self):
        ...
        product = (
            Product.all_objects.select_related(
                "brand",
                "category",
            )
            .prefetch_related(
...
class ProductCacheInvalidationSignalTests(APITestCase):
    ...
    def test_product_post_save_stores_name_in_autocomplete_sorted_set(self):
        ...
        self.assertIn(
            (
                (f"catalog:autocomplete:tenant:{self.tenant.id}:" "product-names"),
                {"MacBook Air": 0},
            ),
...
class ProductAutocompleteEndpointTests(APITestCase):
    ...
    def test_autocomplete_limits_suggestions_to_ten(self):
        self.add_suggestions(
            self.key,
            *[f"Test Product {product_number:02}" for product_number in range(12)],
        )
        self.client.force_authenticate(user=self.user)
...
class ProductListEndpointTests(APITestCase):
    ...
    def test_product_list_endpoint_uses_cache_get_or_set_with_300s_ttl(self):
        ...
        with patch("catalog.services.cache.get_or_set") as get_or_set:
            get_or_set.side_effect = lambda cache_key, default, timeout: default()
...
class ProductDeleteEndpointTests(APITestCase):
    ...
    def test_vendor_can_delete_product(self):
        ...
        self.assertFalse(Product.all_objects.filter(id=self.product.id).exists())
        self.assertFalse(ProductImage.all_objects.filter(id=product_image.id).exists())
    ...
    def test_vendor_cannot_delete_other_tenant_product(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
        self.assertTrue(Product.all_objects.filter(id=other_product.id).exists())
...
class ProductImageUploadEndpointTests(APITestCase):
    ...
    def test_upload_generates_thumbnail_medium_and_large_images(self):
        ...
        self.assertTrue(
            product_image.thumbnail.storage.exists(
                product_image.thumbnail.name,
            )
        )
        self.assertTrue(
            product_image.medium.storage.exists(
                product_image.medium.name,
            )
        )
        self.assertTrue(
            product_image.large.storage.exists(
    ...
    def test_non_vendor_cannot_delete_product_image(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
        self.assertTrue(ProductImage.all_objects.filter(id=product_image.id).exists())
    ...
    def test_vendor_cannot_delete_image_from_other_product(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
        self.assertTrue(ProductImage.all_objects.filter(id=other_image.id).exists())
